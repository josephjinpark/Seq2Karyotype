...
class Genome:
    ...
    def segment_genome (self, m0 = 0, mc = 5.0, fb_alpha = Consts.FB_ALPHA):
        ...
        for segment in self.all_merged_segments:
            # if (1-segment.centromere_fraction)*(segment.end - segment.start) < 5e6 or segment.parameters['model'] == 'UN' or segment.parameters['model'] == 'AB' :
            #     continue
...
def softmax(x):
    neg_scores = -x
    exp_neg_scores = np.exp(neg_scores - np.max(neg_scores,keepdims=True)) # Subtract max for numerical stability
    return exp_neg_scores / exp_neg_scores.sum(keepdims=True)
